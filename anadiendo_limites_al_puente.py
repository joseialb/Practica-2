...
LIMITE_COCHES_PUENTE = 5
LIMITE_PEATONES_PUENTE = 20

# Para una situacion un poco mas cercana a la realidad, vamos a suponer que no puede haber mas de un numero de coches o personas atravesando el puente al mismo tiempo, ya que este tiene un tamaño limitado
# Aqui surge un nuevo problema: En la solucion anterior, todos los de un mismo tipo entraban en el puente a la vez, por lo que no era necesario notificarles cuando otro del mismo tipo salia
# Sin embargo, al añadir un limite de transeuntes simultaneos, puede ocurrir que los que se quedan esperando por que el puente esta lleno no sean nunca notificados.
# Solucion: añadiendo una notificicacion a los que se quedaron esperando cada vez que uno del mismo tipo salga del puente
...
class Monitor():
    ...
    def puede_entrar_N(self) -> bool:
        return self.nVehiS.value == 0 and self.nPeat.value == 0 and self.nVehiN.value < LIMITE_COCHES_PUENTE
    
    def puede_entrar_S(self) -> bool:
        return self.nVehiN.value == 0 and self.nPeat.value == 0 and self.nVehiS.value < LIMITE_COCHES_PUENTE 
    
    def puede_entrar_P(self) -> bool:
        return self.nVehiN.value == 0 and self.nVehiS.value == 0 and self.nPeat.value < LIMITE_PEATONES_PUENTE 
